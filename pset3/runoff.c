...
int main(int argc, string argv[])
{
    ...
    // Print what the ballot looks like after voting is done
    for (int i = 0; i < candidate_count; i++)
    {
        if (i == 0)
        {
        ...
        {
            printf("%s          ", candidates[preferences[j][i]].name);
        }
        printf("\n");
    }
    ...
    while (true)
    {
        ...
        // Try printing the details of each candidate
        printf("   Candidates\t\t Votes\t\t\t Eliminated\n");
        candidate cand;
        for (int i = 0; i < candidate_count; i++)
        {
            cand = candidates[i];
            printf("%i. %s\t\t\t %i\t\t\t %s\n", i + 1, cand.name, cand.votes, cand.eliminated ? "Yes" : "No");
        }
